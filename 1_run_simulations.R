...
gavi_data <- read_csv("scenarios_JT.csv") %>%
  select(scenario, iso3c, country, dec_2021_cov, dec_2022_cov_global_recov) %>%
...
gavi_data$forecast <- as.integer(as.Date("2022-12-31") - as.Date("2021-11-15"))
...
system.time({out <- future_pmap(select(scenarios, -country), create_vacc_fit, .progress = TRUE)})
